...
class _ResConsultasState extends State<ResConsultas> {
  ...
  void initState() {
    ...
    super.initState();
    _motivoController = new TextEditingController(text: widget.consultas.motivos);
  }
              ...
              child: Column(
                children: <Widget>[
                  TextFormField(
                    ...
                    decoration: InputDecoration(icon: Icon(Icons.event_note),
                    labelText: 'Motivo de consukta'
                    ),
                  ),
                  FlatButton(onPressed: (){
                    if(widget.consultas.id!=null){
                      consultasReference.child(widget.consultas.id).set({
